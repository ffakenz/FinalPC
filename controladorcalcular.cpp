    ...
    #pragma omp parallel
    {
        #pragma omp sections
        {
            #pragma omp section
            {
                ...
                if(paraleloA == true){
                    resultadoA =  metodoA->ejecutarParalelo(); qDebug()<<"ParaleloA";
                } else {
                    resultadoA =  metodoA->ejecutarSerial(); qDebug()<<"SerialA";
                }
                #pragma omp critical
            ...
            #pragma omp section
            {
                //int id = omp_get_thread_num();
                if(paraleloB == true){
                    resultadoB =  metodoB->ejecutarParalelo(); qDebug()<<"ParaleloB";
                } else {
                    resultadoB =  metodoB->ejecutarSerial(); qDebug()<<"SerialB";
                }
                #pragma omp critical
            ...
            #pragma omp section
            {
                //int id = omp_get_thread_num();
                if(paraleloC == true){
                    resultadoC =  metodoC->ejecutarParalelo(); qDebug()<<"ParaleloC";
                } else {
                    resultadoC =  metodoC->ejecutarSerial(); qDebug()<<"SerialC";
                }
                #pragma omp critical
            ...
            #pragma omp section
            {
                //int id = omp_get_thread_num();
                if(paraleloD == true){
                    resultadoD =  metodoD->ejecutarParalelo(); qDebug()<<"ParaleloD";
                } else {
                    resultadoD =  metodoD->ejecutarSerial(); qDebug()<<"SerialD";
                }
                #pragma omp critical
